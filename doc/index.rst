...
SubliminalREPL is a plugin for Sublime Text 2 that lets you run interactive
...
SubliminalREPL has a built-in support for command history and transferring code
...
Download `Package Control`__, select Install Package and pick SubliminalREPL from the list of
...
SubliminalREPL adds itself as a submenu in Tools. You can choose any one of the
...
same result as picking it from *Tools > SubliminalREPL* menu.

.. image:: _static/palette.png

.. [#] One of the most frequently reported errors is SubliminalREPL not being able
...
The default shortcuts shipped with SubliminalREPL are listed below. If you are
...
SubliminalREPL's integration with a specific language includes language-specific
...
SubliminalREPL configuration (via Preferences > Package Settings > SubliminalREPL >
...
For virtualenv created environments to be discoverable by SubliminalREPL they should be created or symlinked in one of the following:

* ~/.virtualenvs default for `virtualenvwrapper`__
* ~/.venvs default for `venv`__

Alternatively, more paths can be added to "python_virtualenv_paths" in the SubliminalREPL configuration file.

__ https://bitbucket.org/dhellmann/virtualenvwrapper
__ https://github.com/wuub/venv

Also, you can create custom shells via "custom_python_shells" option in the SubliminalREPL configuration file. They will
...
The default SubliminalREPL configuration documents all available configuration settings.

Frequently Asked Questions
--------------------------

**SubliminalREPL can't launch the REPL process - OSError(2, 'No such file or directory'), how do I fix that?**
   ...
   If the binary is not in your system path and you can't or won't change that, tweak SubliminalREPL configuration::
...
SubliminalREPL currently ships with support for the following languages:
...
A language integration in SubliminalREPL consists of configuration files and,
...
REPLs are started by SubliminalREPL command `repl_open`. The command and its
...
This adds a "Lua" menu item to "Tools > SubliminalREPL" which creates a Lua REPL
via SubliminalREPL command `repl_open`. The important part to take note of here
...
As you can see, the main way to launch a new REPL is the SubliminalREPL command
...
A REPL class is expected to provide a standard interface for SubliminalREPL integration:
